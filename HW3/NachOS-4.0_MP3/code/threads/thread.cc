...
Thread::Yield ()
{
    ...
    // accumulate the real burst time T because the thread jump out of the running state, and get into ready state
    // when the real burst time is changed, the readylist will use the updated real burst time to do the sort again
    ...
    DEBUG(dbgSJF, "[C] Tick [" << kernel->stats->totalTicks << "]: Thread [" << getID() << "] update approximate burst time, from: [" << ti << "], add [" << T << 
    "], to [" << max(0.0, ti-T) << "]");
    // Whenever a process updates its approximate burst time:
    // because the T is changed, so the approximate burst time(remaining burst time) is changed too.
    ...
    if (nextThread != NULL) {
	    DEBUG(dbgSJF, "[E] Tick [" << kernel->stats->totalTicks << "]: Thread [" << nextThread->getID() << 
        "] is now selected for execution, thread [" << getID() << "] is preempted, and it has executed [" << T << "] ticks")
...
Thread::Sleep (bool finishing)
{
    ...
    // use the updated total real burst time T to get the current predict burst time ti
    set_T(0);
    ...
    "] is now selected for execution, thread [" << getID() << "] starts IO, and it has executed [" << 
    T << "] ticks");
